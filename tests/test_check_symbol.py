...
def test_check_symbol_mode(check_symbol, capfd):
    ...
    """
    run_cmake_with_assert(capfd, "cmake-helpers: check_symbol - using check_symbol_exists",
                          {"mode": "check_symbol_exists"})
...
def test_check_symbol_invalid_mode(check_symbol, capfd):
    ...
    with pytest.raises(CalledProcessError):
        run_cmake_with_assert(capfd, "", {"mode": "invalid_mode"})
...
def test_non_existent_symbol(check_symbol, capfd):
    ...
    """
    with pytest.raises(CalledProcessError):
        run_cmake_with_assert(capfd, "cmake-helpers: check_symbol - using check_cxx_symbol_exists", {"symbol": "non_existent_symbol"})